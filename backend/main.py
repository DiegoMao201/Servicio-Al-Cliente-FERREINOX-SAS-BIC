...
def generate_commercial_pdf(
    ...
    # ── Pedido del cliente / resumen operativo ──
    resumen_asesoria = detail.get("resumen_asesoria", "").strip()
    if resumen_asesoria:
        section_title = "Pedido del Cliente" if request_type in {"pedido", "cotizacion"} else "Resumen de la Asesoría"
        summary_lines = [line.strip(" -*•\t") for line in resumen_asesoria.splitlines() if line.strip()]
        summary_html = "<br/>".join(f"• {escape(line)}" for line in summary_lines) if len(summary_lines) >= 2 else escape(resumen_asesoria)
        elements.append(Paragraph(section_title, heading_style))
        elements.append(HRFlowable(width="100%", thickness=0.5, color=brand_border, spaceBefore=1, spaceAfter=3))
        resumen_style = ParagraphStyle(
            "Resumen", parent=normal_style, fontSize=7.8 if compact_mode else 9.2,
            textColor=colors.HexColor("#374151"), leading=10 if compact_mode else 12,
            spaceBefore=2, spaceAfter=2,
        )
        elements.append(Paragraph(summary_html, resumen_style))
        elements.append(Spacer(1, (2 if compact_mode else 4) * mm))
