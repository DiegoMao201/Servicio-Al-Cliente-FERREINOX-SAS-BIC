...
import pandas as pd
import requests
from fastapi import Body, FastAPI, Header, HTTPException, Query, Request, UploadFile, File, BackgroundTasks
...
try:
    ...
    from backend.media_extraction import (
        download_whatsapp_media,
        extract_text_from_pdf_bytes,
        extract_text_from_excel_bytes,
        extract_text_from_image_bytes,
...
# ── Tool handlers (Phase C2 Step 5) — re-exported to preserve main.* API ──
try:
...
def apply_deterministic_product_alias_rules(text_value: Optional[str], prepared_request: dict):
    ...
    alias_rules = [
        ...
        },
        {
            ...
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "mat blanco 10", "3en1 mat blanco 10", "t-10"],
        ...
        },
        {
            ...
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "mat negro 89", "3en1 mat negro", "t-89"],
        ...
        },
        {
            ...
            "preferred_lookup_text": "pintulux gris plata 84",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
        ...
        },
        {
            ...
            "preferred_lookup_text": "pintulux caoba 76",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "caoba 76", "br caoba 76", "t-76"],
            "color_filters": ["caoba"],
        },
        {
            "pattern": r"\b(?:t(?:eu?|u)?)-?\s*80\b",
            "canonical_product": "pintulux verde bronce 80",
            "preferred_lookup_text": "pintulux verde bronce 80",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "verde bronce 80", "br verde bronce", "t-80"],
            "color_filters": ["verde bronce"],
        },
        {
            "pattern": r"\b(?:t(?:eu?|u)?)-?\s*53\b",
            "canonical_product": "pintulux verde esmeralda 53",
            "preferred_lookup_text": "pintulux verde esmeralda 53",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "verde esmeralda 53", "br verde esmer", "t-53"],
            "color_filters": ["verde esmeralda"],
        },
        {
            "pattern": r"\b(?:t(?:eu?|u)?)-?\s*18\b",
            "canonical_product": "pintulux amarillo 18",
            "preferred_lookup_text": "pintulux amarillo 18",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "amarillo 18", "br amarillo 18", "t-18"],
            "color_filters": ["amarillo"],
        },
        {
            "pattern": r"\b(?:t(?:eu?|u)?)-?\s*20\b",
            "canonical_product": "pintulux naranja 20",
            "preferred_lookup_text": "pintulux naranja 20",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "naranja 20", "br naranja 20", "t-20"],
            "color_filters": ["naranja"],
        },
        {
            "pattern": r"\b(?:t(?:eu?|u)?)-?\s*26\b",
            "canonical_product": "pintulux rojo bermellon 26",
            "preferred_lookup_text": "pintulux rojo bermellon 26",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "rojo bermellon 26", "br rojo", "t-26"],
            "color_filters": ["rojo bermellon"],
        },
        {
            "pattern": r"\b(?:t(?:eu?|u)?)-?\s*40\b",
            "canonical_product": "pintulux azul espanol 40",
            "preferred_lookup_text": "pintulux azul espanol 40",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "azul espanol 40", "br azul", "t-40"],
            "color_filters": ["azul espanol"],
        },
        {
            "pattern": r"\b(?:t(?:eu?|u)?)-?\s*12\b",
            "canonical_product": "pintulux crema 12",
            "preferred_lookup_text": "pintulux crema 12",
            "lock_canonical_product": True,
            "clear_product_codes": True,
            "brand_filters": ["pintulux", "pintuco"],
            "core_terms": ["pintulux", "crema 12", "br crema 12", "t-12"],
            "color_filters": ["crema"],
        },
        {
...
# ── Media extraction (WhatsApp/PDF/Excel/Image) extraído a backend.media_extraction en Fase C2 (Paso 4) ──
# Funciones re-exportadas: download_whatsapp_media, extract_text_from_pdf_bytes,
# extract_text_from_excel_bytes, extract_text_from_image_bytes, extract_text_from_media.
...
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 8180..8286
...
# ── RAG search (vector + prefilters + context builder) extraído a backend.rag_search en Fase C2 (Paso 3) ──
# Funciones re-exportadas al inicio de main: _generate_query_embedding, _infer_portfolio_segments_for_query,
# _infer_technical_metadata_prefilters, search_technical_chunks, search_supporting_technical_guides,
# search_multimodal_product_index, fetch_technical_profiles, build_rag_context.
...
def extract_commercial_customer_candidate(text_value: Optional[str]):
    raw_value = (text_value or "").strip()
    if not raw_value:
        return ""

    compact_value = " ".join(raw_value.split()).strip(" ,.;:-")
    normalized_compact = normalize_text_value(compact_value)
    close_signal_tokens = set(re.findall(r"[a-z0-9áéíóúñ]+", normalized_compact))
    if close_signal_tokens and ("pdf" in close_signal_tokens or "cotizacion" in close_signal_tokens):
        allowed_close_tokens = {
            "pdf", "cotizacion", "cotizacion", "la", "el", "en", "de", "del", "por", "favor",
            "envia", "enviar", "manda", "mandar", "genera", "generar", "cotiza", "cotizar",
            "necesito", "quiero", "me", "mi", "porfa"
        }
        if close_signal_tokens.issubset(allowed_close_tokens):
            return ""
    ...
    for pattern in direct_patterns:
        ...
        if not match:
            continue
        candidate = trim_commercial_customer_candidate(match.group(1))
        if candidate and not extract_store_filters(candidate):
            return candidate

    if re.fullmatch(r"\d{6,15}", compact_value):
        return compact_value

    normalized = normalize_text_value(compact_value)
    if normalized and not extract_store_filters(compact_value) and not extract_email_address(compact_value):
        tokens = [token for token in re.findall(r"[a-z0-9áéíóúñ]+", normalized) if len(token) >= 2]
        if 2 <= len(tokens) <= 8 and not is_product_intent_message(compact_value):
            return trim_commercial_customer_candidate(compact_value)
    return ""


def _merge_unique_text_lines(base_lines: list[str], new_lines: list[str]) -> list[str]:
    merged: list[str] = []
    seen: set[str] = set()
    for raw_line in [*(base_lines or []), *(new_lines or [])]:
        clean_line = " ".join(str(raw_line or "").split()).strip(" -*•\t")
        normalized_line = normalize_text_value(clean_line)
        if not clean_line or not normalized_line or normalized_line in seen:
            continue
...
def _is_customer_order_summary_line(text_value: Optional[str]) -> bool:
    clean_value = " ".join(str(text_value or "").split()).strip()
    if not clean_value:
        return False
    if extract_commercial_customer_candidate(clean_value):
        return False
    normalized = normalize_text_value(clean_value)
    if re.search(r"\b(pdf|cotizacion)\b", normalized):
        return False
    if "descuento" in normalized:
        return True
    request = extract_product_request(clean_value)
    return bool(
        request.get("requested_quantity") is not None
        or request.get("requested_unit")
        or request.get("product_codes")
        or request.get("core_terms")
    )


def _build_customer_request_summary(commercial_draft: dict, fallback_summary: Optional[str] = None) -> str:
    raw_lines = [line for line in (commercial_draft.get("raw_request_lines") or []) if line]
    if not raw_lines:
        for item in commercial_draft.get("items") or []:
            if item.get("autogenerated"):
                continue
            # Skip browsing items — only include confirmed/matched items
            if item.get("status") == "browsing":
                continue
            original_text = item.get("original_text")
            if original_text:
                raw_lines.append(str(original_text))
    if not raw_lines and fallback_summary:
        raw_lines.extend(
            line.strip(" -*•\t")
            for line in str(fallback_summary).splitlines()
            if line.strip()
        )
    return "\n".join(_merge_unique_text_lines([], raw_lines))
...
def build_commercial_flow_reply(intent: str, profile_name: Optional[str], user_message: Optional[str], conversation_context: Optional[dict]):
    ...
    if has_existing_items and (wants_quote_pdf or incoming_customer_identity) and not has_contextual_followup and not option_updates:
        current_lines = []
    ...
    if incoming_customer_identity and not has_contextual_followup and not option_updates:
        has_explicit_items = False
        current_lines = []

    # Set items_confirmed when user explicitly confirms
    if is_affirmative_followup or wants_order_confirmation or wants_quote_pdf or incoming_customer_identity:
    ...
    if has_existing_items and (is_affirmative_followup or is_negative_followup or wants_order_confirmation or wants_quote_pdf or incoming_customer_identity) and not has_explicit_items and not has_contextual_followup and not option_updates:
        current_lines = []
    ...
    # Extract destinatario from a short customer-identification follow-up
    if incoming_customer_identity:
        customer_identity_input = incoming_customer_identity
    ...
    requires_store = intent == "pedido"
    has_required_location = has_store or not requires_store
    ready_to_close = all_items_resolved and has_required_location and items_confirmed and not requires_customer_resolution

    if ready_to_close and not incoming_delivery_channel and (wants_order_confirmation or wants_quote_pdf):
        delivery_channel = existing_draft.get("delivery_channel") or "chat"
    elif ready_to_close and intent == "cotizacion" and not delivery_channel:
    ...
    if not ready_to_close:
        ...
        closing_parts = []
        if requires_store and not has_store:
        ...
        elif requires_store and all_items_resolved and not has_store:
    ...
    # ── Ready to close ──
    request_label = "cotización" if intent == "cotizacion" else "pedido"
    destination_label = store_label or ("la sede indicada" if requires_store else "este despacho")
...
def infer_technical_problem_category(text_value: Optional[str], existing_category: Optional[str] = None):
    ...
    metal_surface_tokens = [
        ...
        "techo metalico", "techo metálico", "puerta metalica", "puerta metálica", "escalera metalica", "escalera metálica",
    ]
    ...
    metal_exact_tokens = [token for token in metal_surface_tokens if " " not in token]
    metal_phrase_tokens = [token for token in metal_surface_tokens if " " in token]
    if any(token in normalized for token in [
        ...
        return "metal"
    if any(token in normalized for token in ["madera", "deck", "tabla", "tablas", "tablilla", "mdf", "triplex", "puerta de madera"]):
        return "madera"
    if any(token in normalized for token in metal_phrase_tokens):
        return "metal"
    if any(re.search(rf"(?<![a-z0-9]){re.escape(token)}(?![a-z0-9])", normalized) for token in metal_exact_tokens):
        return "metal"
...
def _infer_technical_current_state(normalized: str) -> Optional[str]:
    if _contains_any_normalized(normalized, ["descascar", "pelad", "soplad", "soplo", "cuartead", "ampollad", "cascari"]):
    ...
    if _contains_any_normalized(normalized, ["oxido", "óxido", "corrosion", "corrosión", "oxidado", "oxidada", "oxidad", "oxidand"]):
        return "con oxido o corrosion"
    if _contains_any_normalized(normalized, ["grasa", "aceite", "gasolina", "diesel", "diésel", "combustible", "contaminada", "sucia"]):
...
def extract_technical_advisory_case(text_value: Optional[str], conversation_context: Optional[dict]):
    ...
    metal_substrate_phrase_tokens = [
        "teja de zinc", "teja zinc", "techo metalico", "techo metálico", "puerta metalica", "puerta metálica",
    ]
    metal_substrate_exact_tokens = [
        "metal", "metalico", "metalica", "reja", "rejas", "baranda", "barandas", "porton", "portón", "acero",
        "hierro", "galvanizado", "aluminio", "tanque", "lamina", "lámina",
    ]
    if any(token in normalized for token in metal_substrate_phrase_tokens) or any(
        re.search(rf"(?<![a-z0-9]){re.escape(token)}(?![a-z0-9])", normalized)
        for token in metal_substrate_exact_tokens
    ):
        case["substrate_type"] = "metal"
    elif any(token in normalized for token in ["fibrocemento", "eternit"]):
        case["substrate_type"] = "fibrocemento"
    ...
    elif any(token in normalized for token in ["madera", "deck", "tabla", "tablas", "tablilla", "mdf", "triplex"]):
    ...
    elif any(token in normalized for token in ["interior", "adentro", "por dentro", "bajo techo", "baño", "bano", "cocina"]):
        case["exposure_environment"] = "interior"
    elif any(token in normalized for token in ["exterior", "intemperie", "sol", "lluvia", "fachada", "terraza", "jardin", "jardín", "techo", "rooftop", "pergola", "pérgola", "lago"]):
    ...
    if category == "humedad":
        if any(token in normalized for token in ["barranco", "terreno", "talud", "contencion", "contención"]):
            ...
            case["probable_pressure"] = "presion_negativa"
        elif any(token in normalized for token in ["terraza", "cubierta", "techo", "azotea", "placa"]):
            case["source_context"] = "posible filtracion desde terraza o cubierta"
            case["probable_pressure"] = "presion_negativa"
        elif any(token in normalized for token in ["tuberia", "tubería", "tubo", "fuga"]):
            case["source_context"] = "posible tuberia o fuga interna"
        elif any(token in normalized for token in ["fachada", "lluvia", "exterior", "afuera", "por fuera", "jardin", "jardín"]):
        ...
        if any(token in normalized for token in ["por dentro", "desde adentro", "interior", "adentro", "dentro de la casa", "casa", "sotano", "sótano"]):
        ...
        if any(token in normalized for token in ["exterior", "afuera", "intemperie"]):
            case["floor_location"] = "exterior"
        elif any(token in normalized for token in ["parqueadero", "anden", "andén"]):
            case["floor_location"] = "exterior"
        elif any(token in normalized for token in ["interior", "adentro", "bajo techo"]):
            case["floor_location"] = "interior"
        elif any(token in normalized for token in ["bodega", "local", "taller"]):
            case["floor_location"] = "interior"

        if any(token in normalized for token in ["cemento", "concreto", "mortero"]):
            case["floor_material"] = "cemento o concreto"
        elif any(token in normalized for token in ["garaje", "parqueadero", "bodega", "montacargas", "obra gris", "taller", "motos"]):
            case["floor_material"] = "cemento o concreto"
        ...
        if any(token in normalized for token in ["alto trafico", "alto tráfico", "montacarga", "vehiculo", "vehículo", "carro", "llanta", "llantas", "todo el tiempo", "todos los dias", "todos los días"]):
            case["traffic_level"] = "alto trafico"
        elif any(token in normalized for token in ["taller", "motos"]):
            case["traffic_level"] = "alto trafico"
        elif any(token in normalized for token in ["garaje", "parqueadero"]):
            case["traffic_level"] = "trafico vehicular"
        elif any(token in normalized for token in ["peatonal", "peaton", "peatones", "residencial", "casa", "habitacion", "habitación"]):
        ...
        case["exposure_environment"] = "exterior"
        if case.get("current_state") == "con recubrimiento deteriorado":
            pass
        elif any(token in normalized for token in ["pintura vieja", "repint", "ya pint", "graniplast", "silcoplast", "textura", "recubrimiento", "descascar", "soplad"]):
        ...
        if any(token in normalized for token in ["intemperie", "exterior", "sol", "lluvia", "rooftop", "pergola", "pérgola", "lago", "piscina"]):
            case["exposure"] = "intemperie"
        elif any(token in normalized for token in ["interior", "bajo techo", "adentro"]):
            case["exposure"] = "bajo techo"
        elif any(token in normalized for token in ["baño", "bano", "ducha", "cafeteria", "cafetería", "cocina"]):
            case["exposure"] = "bajo techo"

        if any(token in normalized for token in ["barniz", "laca", "pintada", "pintado", "pintura", "tiene recubrimiento", "ya tiene"]):
            case["previous_coating"] = "con recubrimiento previo"
        ...
        elif any(token in normalized for token in ["reja", "rejas", "baranda", "tanque", "puerta metalica", "puerta metálica", "escalera", "escaleras"]):
            case["metal_type"] = "metal ferroso"
        ...
        if any(token in normalized for token in ["ya pint", "repint", "esmalte", "anticorrosivo", "alquid", "epox", "epóx", "primer", "base vieja", "tuvo pintura", "ya tuvo pintura"]):
            case["previous_coating"] = "con recubrimiento previo"
        elif any(token in normalized for token in ["metal desnudo", "sin pintar", "nuevo", "nueva"]):
            case["previous_coating"] = "sin recubrimiento previo"

        if not case.get("exposure_environment") and any(token in normalized for token in ["sol", "lluvia", "intemperie", "jardin", "jardín", "exterior", "techo", "playa"]):
            case["exposure_environment"] = "exterior"
        elif not case.get("exposure_environment") and any(token in normalized for token in ["interior", "adentro", "por dentro", "bajo techo", "local", "bodega", "apto", "apartamento", "locker"]):
            case["exposure_environment"] = "interior"
...
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 12981..13034
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 13035..13089
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 13090..13150
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 13151..13168
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 13169..13241
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 13242..13280
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 13281..13343
...
# [moved-to-rag_helpers.py @ Phase C3 HITO 1] originally lines 13515..13602
...
def _draft_items_to_confirmation_payloads(commercial_draft: dict) -> list[dict]:
    payload_items = []
    for item in commercial_draft.get("items") or []:
        if item.get("status") not in ("matched", "browsing"):
            continue
...
def generate_commercial_pdf(
    ...
    return generate_commercial_pdf_v2(
        ...
        cliente_contexto,
        detail,
        price_resolver=_resolve_pdf_line_pricing,
        store_labels=STORE_CODE_LABELS,
    )
...
# [moved-to-customer_handlers.py @ Phase C3 HITO 3] originally lines 16814..16909
# [moved-to-customer_handlers.py @ Phase C3 HITO 3] originally lines 16910..17160
...
def _handle_tool_confirmar_pedido(args, context, conversation_context):
    ...
    if _using_authoritative_draft:
        for it in items_pedido:
            _desc = it.get("descripcion_comercial") or it.get("descripcion_exacta") or ""
            _ref = it.get("referencia") or ""
            _qty = it.get("cantidad") or 1
            _unit = it.get("unidad_medida") or "unidad"
            confirmed_items.append({
                "status": "matched",
                "source": "draft",
                "original_text": _desc,
                "descripcion_comercial": _desc,
                "descripcion_exacta": it.get("descripcion_exacta") or _desc,
                "referencia": _ref,
                "codigo_articulo": _ref,
                "cantidad": _qty,
                "unidad_medida": _unit,
                "audit_label": it.get("audit_label") or it.get("etiqueta_auditable") or _desc,
                "etiqueta_auditable": it.get("etiqueta_auditable") or it.get("audit_label") or _desc,
                "matched_product": {
                    "referencia": _ref,
                    "codigo_articulo": _ref,
                    "descripcion": _desc,
    ...
    # Build summary AFTER overwriting items so only confirmed products appear
    customer_request_summary = _build_customer_request_summary(commercial_draft, resumen_asesoria)
    ...
    commercial_draft["resumen_asesoria"] = customer_request_summary or resumen_asesoria or ""
    ...
    commercial_draft["raw_request_lines"] = _merge_unique_text_lines(
        commercial_draft.get("raw_request_lines") or [],
        [line for line in customer_request_summary.splitlines() if line.strip()],
    )
...
def _execute_agent_tool(tool_call, context, conversation_context):
    ...
    logger.info("Tool %s completed in %dms | args=%s | result=%s", fn_name, elapsed_ms, json.dumps(fn_args, ensure_ascii=False)[:200], result_preview)
...
@app.get("/admin/bi-diagnostico")
def admin_bi_diagnostico(admin_key: str = Header(None, alias="x-admin-key")):
    """Diagnóstico BI: valida tablas, rangos de fechas, funciones y queries clave."""
    expected = os.getenv("ADMIN_API_KEY", "ferreinox_admin_2024")
    if admin_key != expected:
        raise HTTPException(status_code=403, detail="Admin key inválida")
    engine = get_db_engine()
...
# ── [Phase C3 HITO 1] Re-exports desde rag_helpers.py ────────────────────
# Mantiene API pública: main._build_structured_diagnosis(...), main.fetch_expert_knowledge(...), etc.
try:
...
# ── [Phase C3 HITO 3] Re-exports desde customer_handlers.py ───────────────
# Mantiene API pública: main._handle_tool_verificar_identidad(args, ctx, conv)
try:
    ...
    from backend.customer_handlers import (
        _handle_tool_verificar_identidad,
        _handle_tool_registrar_cliente_nuevo,
    )
